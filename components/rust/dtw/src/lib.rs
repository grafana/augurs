...
use bindings::exports::augurs::dtw::dtw::{
    DistanceMatrix, Dtw, DtwOpts, Guest, GuestDtw, TimeSeries,
};
...
impl GuestDtw for DtwWrapper {
    fn euclidean(opts: Option<DtwOpts>) -> Dtw {
        ...
        if let Some(window) = opts.and_then(|x| x.window) {
    ...
    fn distance_matrix(&self, series: Vec<TimeSeries>) -> DistanceMatrix {

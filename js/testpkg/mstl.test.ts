...
import { ets, MSTL, initSync } from '@bsull/augurs/mstl';

import { describe, expect, it } from 'vitest';

initSync({ module: readFileSync('node_modules/@bsull/augurs/mstl_bg.wasm') });
...
describe('ets', () => {
  ...
  it('can be instantiated with a (deprecated) standalone function', () => {
    // @ts-ignore
    const model = ets([10]);
    expect(model).toBeInstanceOf(MSTL);
  });

//! Standard series transformations.
//!
//! Includes differencing, seasonal differencing and Box-Cox transformation.
...
pub fn diff(y: &[f64], d: usize) -> Result<Vec<f64>> {
  ...
  let n = y.len();
  if n <= d {
    return Err(Error::SeriesTooShort { need: d + 1, got: n })
  }
...
pub fn seasonal_diff(y: &[f64], period: usize, sd: usize) -> Result<Vec<f64>> {
  ...
  let n = y.len();
  if n <= period * sd {
    return Err(Error::SeriesTooShort { need: period * sd + 1, got: n })
  }

  let mut y_vec = y.to_vec();
  for _ in 0..sd {
    y_vec = y_vec.iter().skip(period).zip(y_vec.iter()).map(|(a, b)| a - b).collect();
  }
...
mod tests {
  ...
  fn test_seasonal_diff_1() {
    ...
    assert_eq!(y_diff.unwrap(), vec![6.0, 11.0, 13.0, -1.0, -2.0, -9.0, -13.0])
  }
